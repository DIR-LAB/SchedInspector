...
def plot_data(data, xaxis='Epoch', value="AverageEpRet", condition="Condition1", smooth=1, **kwargs):
    ...
    a = sns.tsplot(linewidth=3, data=data, time=xaxis, value=value, unit="Unit", condition=condition, ci='sd', **kwargs)
    #ax2 = plt.twinx()
    #b = sns.tsplot(linewidth=3, data=data, time=xaxis, color="sandybrown", legend=None, value="SkipRatio", ax=ax2, unit="Unit", condition=None, ci='sd', **kwargs)
    a.set(ylabel="Metrics Improvement")
    # #a.set(ylabel="Delay Ratio")
    #b.set(ylabel="Delay Ratio")
    # #b.spines['right'].set_color("red")
    #b.yaxis.label.set_color('sandybrown')
    #b.tick_params(axis='y', colors='sandybrown')
    """
    ...
    #plt.xlim([80,100])
    #plt.ylim([-250,80])
    #plt.yticks(range(-250, 51, 50))
    #plt.ylim([0,400])

    plt.legend(loc=4, prop={'size':23}).set_draggable(True)
    #plt.legend(loc=4, prop={'size':18}).set_draggable(True)
    ...
    plt.legend(loc='upper center', ncol=6, handlelength=1,
               mode="expand", borderaxespad=0., prop={'size': 23})
    """
...
def make_plots(all_logdirs, legend=None, xaxis=None, values=None, count=False,
               font_scale=1.5, smooth=1, select=None, exclude=None, estimator='mean', other_algos=False, save=0):
    ...
    if save:
        imgname = all_logdirs[0][:-1]+"_"+values[0]+".png"
        print(imgname)
        plt.savefig(imgname)
    else:
        plt.show()
...
traj_per_epoch
    ...
    make_plots(args.logdir, args.legend, args.xaxis, args.value, args.count, 
               ...
               estimator=args.est, other_algos=args.other_algos, save=args.save)
